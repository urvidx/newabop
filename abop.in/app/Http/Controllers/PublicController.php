...
class PublicController extends Controller
{
    ...
    public function showUserProfile($slug)
    {
        ...
        if (!empty($user)) {
            ...
            if ($user->getRoleNames()->first() === 'boutique') {
                $services = array();
    ...
    public function getSearchResult($search_type = "")
    {
        ...
        // Make fixed search to Boutiques(freelancers)
        $type = !empty($_GET['type']) ? $_GET['type'] : 'boutique';
        //$type = 'boutique';
        ...
        $categories2 = Category::pluck('title', 'id');
        // if (!empty($_GET['type'])) {
        if (!empty($type)) {
            if ($type == 'employer' || $type == 'boutique') {
                    ...
                    $e_inner_banner = !empty($inner_page) && !empty($inner_page[0]['e_inner_banner']) ? $inner_page[0]['e_inner_banner'] : null;
                        ...
                        return view(
                            ...
                            compact(
                                ...
                                'e_inner_banner',
                                'enable_package',
                                'show_breadcrumbs','job_duration','categories2'
                            )
                        );
    ...
    {
        $json = array();
        if (Auth::user() && Auth::user()->getRoleNames()->first() === 'boutique') {
            $json['auth'] = true;
            return $json;
        } else {
            $json['auth'] = false;
