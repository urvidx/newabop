...
class JobController extends Controller
{
    ...
    public function postJobFromHome(Request $request)
    {
        if ($request->ajax()) {
            ...
            //print_r($ipAddr);
            $geoIP  = json_decode(file_get_contents("http://ip-api.com/json/".$ipAddr), true);

            // Check Auth User if logins or not
            if(\Auth::check()){
                $user = \Auth::user();
            }else{
                $phoneNumber = $request->phoneNumberPost;

                $fyndUser = User::where('mobile_number',$request->phoneNumberPost)->first();

                if ($fyndUser =='' || empty($fyndUser)) {
            ...
            $reqArr = array();
            $reqArr['title'] = $request->title;
            $reqArr['job_duration'] = $request->title;
            $reqArr['project_levels'] = 'basic';
            $reqArr['freelancer_type'] = 'independent';
            $reqArr['english_level'] = 'basic';
            $reqArr['project_type'] = 'fixed';
            $reqArr['price'] = $request->title;
            $reqArr['description'] = $request->title;
            $reqArr['address'] = $request->address;
            $reqArr['longitude'] = $geoIP['lon'];
            $reqArr['latitude'] = $geoIP['lat'];
            $reqArr['locations'] = 4;
            $reqArr['project_cost'] = $request->price;
            $reqArr['is_featured'] = true;
            $reqArr['show_attachments'] = true;
            $reqArr['expiry_date'] = null;
            $reqArr['skills'] = null;
            $reqArr['languages'] = array('13');
            $reqArr['categories'] = $request->categories;
            
            $job_post = $this->job->storeJobs($reqArr);
    ...
    function createUser($phone)
    {
            ...
            $reqarr = array();
            $reqarr['first_name'] = 'UserName';
            $reqarr['last_name'] = 'Abop';
            $reqarr['password'] = 'usernameabop';
            $phne = substr($phone, -5);
            $createEmail = 'UserNameAbop'.$phne.'@abop.com';
            $reqarr['email'] = $createEmail;
            $reqarr['role'] = 'employer';

            //echo "<pre>";print_r($reqarr);die;
            $json = array();
            $user = new User();

            $user_id = $user->storeUser($reqarr, $verification_code, $registration_type, $verification_type);
            session()->put(['user_id' => $user_id]);
            session()->put(['email' => $createEmail]);
            session()->put(['password' => 'usernameabop']);

            $id = Session::get('user_id');
            User::where('id',$id)->update(['mobile_number'=>$phone]);
