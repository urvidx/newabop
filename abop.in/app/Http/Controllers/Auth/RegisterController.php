...
class RegisterController extends Controller
{
    ...
    */
    ...
    public function registerAsClient(Request $request)
    {
        if ($request->ajax()) {
            ...
            $reqarr = array();
            $reqarr['first_name'] = $request->first_name;
            $reqarr['last_name'] = $request->last_name;
            $reqarr['password'] = $request->password;
            if (empty($request->email) && $request->email == '') {
                $phne = substr($request->mobile_number, -5);
                $createEmail = $request->first_name.$request->last_name.$phne.'@abop.com';
                $reqarr['email'] = $createEmail;
            }else{
                $reqarr['email'] = $request->email;
            }
