    ...
    public static function getSearchResult(
        ...
        if (!empty($users)) {
            ...
            if (!empty($keyword)) {
                ...
                $userprofiles1 = Profile::where('tagline', 'like', '%' . $keyword . '%')->get()->pluck('user_id')->toArray();
                $userprofiles2 = Profile::where('description', 'like', '%' . $keyword . '%')->get()->pluck('user_id')->toArray();
                $sss = array_merge(@$userprofiles1,@$userprofiles2);
                $rrr = array_unique(@$sss);

                $users->where(function($q) use ($keyword){
                    $q->where('first_name', 'like', '%' . $keyword . '%')->orwhere('last_name', 'like', '%' . $keyword . '%')->orWhere('slug', 'like', '%' . $keyword . '%');
                })->orWhere(function($l) use ($rrr){
                    $l->whereIn('id', $rrr);
                });
                ...
                // $users->where('is_disabled', 'false');
                 //echo "<pre>";print_r($user_by_role);//die;
                //  $userprofiles1 = Profile::where('tagline', 'like', '%' . $keyword . '%')->get()->pluck('user_id')->toArray();

                //  $userprofiles2 = Profile::where('description', 'like', '%' . $keyword . '%')->get()->pluck('user_id')->toArray();

                //  //echo "<pre>";print_r($userprofiles1);//die;
                //  $sss = array_merge(@$userprofiles1,@$userprofiles2,@$user_by_role);

                //  //echo "<pre>";print_r($sss);//die;
                //  $rrr = array_unique(@$sss);
                //  echo "<pre>";print_r($users->get());die;
                // echo "<pre>";print_r($rrr);die;
                // // $users->whereIn('id',@$rrr);
                // $users->whereIn('id', $rrr);
                // $users->where('is_disabled', 'false');
