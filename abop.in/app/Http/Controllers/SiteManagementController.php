...
class SiteManagementController extends Controller
{
    ...
    public function homePageSettings()
    {
        ...
        $sliderImgs = Slider::get();
        $workdetails = WorkDetail::get();
            return view(
                ...
                compact(
                    ...
                    'service_sec_subtitle', 'service_sec_description','sliderImgs','workdetails'
                )
            );
    ...
    {   
        ...
        $randmStr = substr(str_shuffle($str_result), 0, 10);
        if ($files = $request->file('image')){
            // for save original image
            $ImageUpload = \Image::make($files);
            $originalPath = public_path('/sliderImages/');
            $ImageUpload->save($originalPath.$randmStr.$files->getClientOriginalName());

            // for save thumnail image
            $thumbnailPath = public_path('/sliderImages/thumbnail/');
            $ImageUpload->resize(250,125);
            $ImageUpload = $ImageUpload->save($thumbnailPath.$randmStr.$files->getClientOriginalName());

            $slider = new Slider();
            $slider->title = $request->title;
            $slider->description = $request->description;
            $slider->image = $randmStr.$files->getClientOriginalName();
            $slider->created_at = date('Y-m-d h:i:s');
            $slider->updated_at = date('Y-m-d h:i:s');
            $slider->save();
            $image = Slider::latest()->first(['id','image','title','description']);
    ...
    public function editSliderImage(Request $request){
        ...
        $str_result = '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
        $randmStr = substr(str_shuffle($str_result), 0, 10);
        $oldDetail = Slider::where('id',$request->id)->first();
        $image = $oldDetail->image;
        if ($files = $request->file('image')){
            // for save original image
            $ImageUpload = \Image::make($files);
            $originalPath = public_path('/sliderImages/');
            ...
            $ImageUpload->resize(250,125);
            $ImageUpload = $ImageUpload->save($thumbnailPath.$randmStr.$files->getClientOriginalName());
            $image = $randmStr.$files->getClientOriginalName();
        ...
        $slider->updated_at = date('Y-m-d h:i:s');
        $slider->save();
        $image = Slider::where('id',$request->id)->first(['id','image','title','description']);
        return Response()->json($image);
    }
    ...
    public function saveWorkDetail(Request $request)
    {
        ...
        if ($files = $request->file('image')){
            // for save original image
            $ImageUpload = \Image::make($files);
            $originalPath = public_path('/uploads/workdetails/');
            $ImageUpload->save($originalPath.$randmStr.$files->getClientOriginalName());

            // // for save thumnail image
            // $thumbnailPath = public_path('/uploads/workdetails/thumbnail/');
            // $ImageUpload->resize(250,125);
            // $ImageUpload = $ImageUpload->save($thumbnailPath.time().$files->getClientOriginalName());

            $icon = $randmStr.$files->getClientOriginalName();
            //return 'ok';
        }

        $slider = new WorkDetail();
        $slider->title = $request->title;
        $slider->description = $request->description;
        $slider->icon = $icon;
        $slider->created_at = date('Y-m-d h:i:s');
        $slider->updated_at = date('Y-m-d h:i:s');
        $slider->save();
        $image = WorkDetail::latest()->first(['id','icon','title','description']);
        return Response()->json($image);
    }
    ...
    public function updateWorkDetail(Request $request){
        ...
        $oldWorkDetail = WorkDetail::where('id',$request->id)->first();
        $icon = $oldWorkDetail->icon;
        $str_result = '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
        $randmStr = substr(str_shuffle($str_result), 0, 10);
        if ($files = $request->file('image')){
            // for save original image
        ...
        $slider = WorkDetail::find($request->id);
        $slider->title = $request->title;
        $slider->description = $request->description;
        $slider->icon = $icon;
        $slider->updated_at = date('Y-m-d h:i:s');
        $slider->save();
        $image = WorkDetail::where('id',$request->id)->first(['id','icon','title','description']);
        return Response()->json($image);
    }

    public function deleteWorkImage(Request $request,$id){
        $slider = WorkDetail::where('id',$id)->delete();
    }
    public function deleteSliderImage(Request $request)
    {
        ...
        // if (!empty($slider)) {
        //     //unlink(public_path('/sliderImages/thumbnail/'.$slider->image));
        //     //unlink(public_path('/sliderImages/'.$slider->image)); 
